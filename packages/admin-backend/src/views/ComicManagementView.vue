...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      comicsCount.value = (await http.getComics()).data.length
      unfilledComics.value = (await http.getComics()).data.filter(comic => comic.chapters === 0)
      unfilledComicsCount.value = unfilledComics.value.length
      console.log(unfilledComics.value)
    })
  ...
  <main class="absolute top-20 w-full px-4">
    ...
    <div class="mt-6 px-4 flex items-center">
      <div class="mx-auto flex items-center gap-3">
        <input type="text" placeholder="请输入关键字" class="input input-primary mr-auto block" v-model="keyword">
    ...
    <div class="w-full px-4 mt-4">
      <div class="bg-base-300/25 w-full py-4 box-border" v-for="comic in foundComics" :key="comic._id" @click="selectComic(comic)">
    ...
    <div class="w-full px-4 mt-4" v-show="unfilledComicsShow">
      <div class="bg-base-300/25 w-full py-4 box-border" v-for="comic in unfilledComics.slice(0, 5)" :key="comic._id" @click="selectComic(comic)">
        <RouterLink :to="{name: 'comic-detail', params: { id: comic._id }}">
          <div class="mx-auto flex flex-col w-5/6 navbar btn bg-base-300 rounded-lg p-0">
            <h1 class="text-lg">{{comic.name}}</h1>
            <h3>{{comic.author}}</h3>
        ...
        <div class="card">
          <form class="card-body">
            <div class="form-control space-y-3">
              <input type="text" placeholder="漫画名字" class="input input-bordered" v-model="name" required />
              <input type="text" placeholder="作者" class="input input-bordered" v-model="author" required />
